"""Edge fetcher for eia_gasoline (Session 2B, Group A) — EIA weekly gasoline via FRED.
...
def fetch(as_of: str | None = None) -> Path:
    ...
    print(f"eia_gasoline: {len(rows)} weekly rows  {rows[0]['period']}..{rows[-1]['period']}")
    n = _ingest.load(spec["source"], staged, prov_path, as_of)
    print(f"eia_gasoline: loaded {n} rows")
    return staged
