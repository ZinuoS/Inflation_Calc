...
**Report date: 2026-07-30** · regenerate daily with
`python -c "from nowcast import pce_status; pce_status.update()"` · generated by
`src/nowcast/pce_status.py`

**PRINT PUBLISHED.** First-release core PCE MoM = **+0.132 pp** (+13.2 bp). This report does NOT adjudicate — run `report.postmortem('pce','2026-06-01')` per the runbook to populate the ledger.
...
| realized | **+13.2** |
| verdict | **HIT (tenth +0.1% called and realized; -5.6bp, inside +/-8.0 band)** |
...
| 2026-07-30 | 0 | pending | +7.6 | print not yet published; call stands frozen |
